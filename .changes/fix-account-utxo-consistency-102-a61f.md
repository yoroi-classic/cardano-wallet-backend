...
- Multi-page Koios account and address UTxO reads now page from a composite `tx_hash`/`tx_index`
  ...
  same-total snapshot with a silently shifted row. Reads that still page by offset verify ordered
  membership with a second walk and retry when the two disagree.
