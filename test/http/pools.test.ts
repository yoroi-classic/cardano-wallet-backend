...
describe('pools list route — paging is validated, not coerced', () => {
  // z.coerce.number() runs the query string through JS Number(), which accepts far more
  // than a page bound should. Each of these was silently accepted before.
  it.each([
    ...
    const res = await app.inject({ method: 'GET', url: `/v1/pools${query}` })
